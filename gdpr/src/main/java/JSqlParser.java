                        ...
                        if(index instanceof ForeignKeyIndex){
                            ...
                            for(String columnName:fk.getColumnsNames()){
                                if(colsWithUnique.contains(columnName)){
                                    ForeingKey foreingKey=new ForeingKey(fk.getTable().getName(),fk.getReferencedColumnNames() , true);
                                    table.addForeingKeyConst(foreingKey);
                                   
                                }
                                else{
                                     ForeingKey foreingKey=new ForeingKey(fk.getTable().getName(),fk.getReferencedColumnNames() , false);
                                    table.addForeingKeyConst(foreingKey);
...
CREATE TABLE Event_Ticket_Assignment (
    ...
    FOREIGN KEY (ticket_ID) REFERENCES Event_Tickets(ticket_ID)
                ...
                """;
        Graph graph=new Graph(Parser(query));
        graph.annotate("users");


        /*
           Graph graph=new Graph(Parser(query1));
           graph.annotate("Attendee");


           Graph graph=new Graph(Parser(sql));
           graph.annotate("Users");
